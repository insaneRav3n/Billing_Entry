<?php
	...
	*/
	 ...
	 */
		...
		{
			$request = $this->dbh->prepare("SELECT * FROM customers  ORDER BY id DESC");
			if ($request->execute()) {
				return $request->fetchAll();
			}
			return false;
		...
		public function updateDue($vala)
		{
			$request = $this->dbh->prepare("UPDATE customers SET due=?");
			return $request->execute([$vala]);
		}
	...
	*/
		...
		{
			$request = $this->dbh->prepare("DELETE FROM billings WHERE id = ?");
			return $request->execute([$id]);
		}     
	/**
	 ...
	 */
		...
		*/
		...
		public function getCustomer_find($name)
		{
			
			$request = $this->dbh->prepare("SELECT * FROM customers WHERE full_name like  CONCAT('%',?,'%')");
		...
		*/
		...
		public function invPay( $customer_id, $bill_date, $bill_amount )
		{	
			try {
				$this->dbh->beginTransaction();
					$request = $this->dbh->prepare("INSERT INTO billings (customer_id, bill_date,  bill_amount, paid_on) VALUES(?,?,?,NOW())");
					$request->execute([$customer_id, $bill_date, $bill_amount]);
	...
	// Bill generation of a Month
		...
		public function fetchBills($from,$to){
			$request = $this->dbh->prepare("SELECT * FROM `billings` where paid_on BETWEEN ? AND ?");
			if ($request->execute([$from,$to])) {
				return $request->fetchAll();
			}
			return false;
               ...
               public function fetchSum($from,$to){
			$request = $this->dbh->prepare("SELECT SUM(bill_amount) AS TotalSum FROM `billings` where paid_on BETWEEN ? AND ?");
			if ($request->execute([$from,$to])) {
				return $request->fetchAll();
			}
			return false;
